...
func PreviousAsset(baseConfig *config.Config, com *common.Common) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		var lastHistoryEntryIndex int

		for i, h := range requestConfig.History {
			if strings.HasPrefix(h, "*") {
				requestConfig.History[i] = strings.Replace(h, "*", "", 1)
				lastHistoryEntry = requestConfig.History[i-1]
				lastHistoryEntryIndex = i - 1
				break
			}
		}
		...
		if lastHistoryEntry == "" {
			...
			lastHistoryEntryIndex = historyLen - 2
		}

		prevAssets := strings.Split(lastHistoryEntry, ",")
		requestConfig.History[lastHistoryEntryIndex] = "*" + requestConfig.History[lastHistoryEntryIndex]
...
func NextAsset(baseConfig *config.Config, com *common.Common, c echo.Context) error {
	...
	var nextHistoryEntry string
	var nextHistoryEntryIndex int

	for i, h := range requestConfig.History {
		if strings.HasPrefix(h, "*") {
			requestConfig.History[i] = strings.Replace(h, "*", "", 1)
			nextHistoryEntry = requestConfig.History[i+1]
			nextHistoryEntryIndex = i + 1
			break
		}
	}

	if nextHistoryEntry == "" {
		nextHistoryEntry = requestConfig.History[historyLen-1]
		nextHistoryEntryIndex = historyLen - 1
	}

	nextAssets := strings.Split(nextHistoryEntry, ",")
	requestConfig.History[nextHistoryEntryIndex] = "*" + requestConfig.History[nextHistoryEntryIndex]

	viewData := common.ViewData{
		KioskVersion:  KioskVersion,
		RequestID:     requestID,
		DeviceID:      deviceID,
		Assets:        make([]common.ViewImageData, len(nextAssets)),
		Queries:       c.QueryParams(),
		UpdateHistory: false,
		Config:        requestConfig,
	}
	...
	for i, assetID := range nextAssets {
